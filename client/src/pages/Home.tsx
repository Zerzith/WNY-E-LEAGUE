...
// Add a simple Marquee component for news
const NewsMarquee = ({ news }: { news: News[] }) => {
  if (news.length === 0) return null;

  const marqueeText = news.map(item => item.title).join(" \u2022 "); // Join news titles with a bullet point
...
export default function Home() {
  ...
  useEffect(() => {
    // Fetch all upcoming/open events
    const qEvents = query(
...
const NewsMarquee = ({ news }: { news: News[] }) => {
  ...
  return (
    <div className="w-full bg-primary/10 py-2 overflow-hidden relative">
